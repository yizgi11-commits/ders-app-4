...
export default function StepDailyGoal({ value, onChange, onNext, onBack }: Props) {
  return (
    ...
    >
      ...
      >
        Günde kaç saat çalışmayı hedefliyorsun? ⏰
      </motion.h2>
      <p className="text-sm text-white/35 mb-8">Günlük hedeflerini ve programını buna göre oluşturacağız.</p>

      <div className="grid grid-cols-2 gap-3 mb-8">
        {DAILY_GOAL_OPTIONS.map((opt, i) => (
          <motion.button
            key={opt.value}
            ...
            whileTap={{ scale: 0.95 }}
            className={cn(
              'flex items-center justify-center gap-2.5 p-5 rounded-xl border transition-all',
              value === opt.value
                ? 'bg-indigo-500/15 border-indigo-500/30'
                : 'bg-white/[0.03] border-white/[0.07] hover:border-white/[0.14]',
            )}
          >
            <Clock className={cn('w-4 h-4', value === opt.value ? 'text-indigo-300' : 'text-white/25')} />
            <span className={cn(
              'text-base font-bold',
              value === opt.value ? 'text-indigo-300' : 'text-white/60',
            )}>
              {opt.label}
            </span>
          </motion.button>
