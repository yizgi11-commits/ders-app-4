...
import { Lock, Loader2, RotateCcw, BookmarkPlus, Check, Send } from 'lucide-react'
...
interface Props {
  ...
  /** Free text is Pro-only — Free sees a locked input. */
...
export default function AssistConversation({
  pageContext, introText, quickActions = [], onSave, tier,
}: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Free text — Pro-only. Contextual buttons above cover every
          supported action for Free without needing typed input. */}
      {tier === 'pro' ? (
        ...
        <Link href="/dashboard/upgrade" className="block p-3 border-t border-border group">
          <div className="flex items-center gap-2">
            <div className="flex-1 text-sm bg-gray-50 border border-border rounded-xl px-3.5 py-2.5 text-muted-foreground group-hover:bg-gray-100 transition-colors cursor-pointer">
